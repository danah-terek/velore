...
const orderService = {
  async createOrder(userId, { address_id, payment_method, email, name, phone, address: guestAddress, guestItems, discount_amount ,discount_code}) {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (discount_code) {
        const { incrementUsage } = require('../discounts/discount.service')
        await incrementUsage(discount_code)
      }
      return {
        order,
        orderNumber,

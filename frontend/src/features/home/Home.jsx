...
export default function Home() {
  ...
  const loadNewCollection = async () => {
    try {
      const response = await shopService.getProducts({ limit: 5 })
      setNewProducts(response.data.slice(0, 5))
    } catch (error) {
          ...
          <>
            ...
            <div className="flex md:hidden gap-3 overflow-x-auto pb-4 scrollbar-hide">
              {newProducts.map((product) => (
                <div key={product.productId} className="w-[45vw] flex-shrink-0">
                  <EyewearCard {...product} />
                </div>
              ))}
            </div>
            ...
            <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {newProducts.map((product) => (
                <EyewearCard key={product.productId} {...product} />
              ))}
            </div>
          </>
        )}
